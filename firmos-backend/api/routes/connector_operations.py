    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            """SELECT i.id,i.client_id,i.provider,i.display_name,i.status,i.configuration,
              ...
              (SELECT c.expires_at FROM connector_credentials c WHERE c.installation_id=i.id AND c.revoked_at IS NULL ORDER BY c.credential_version DESC LIMIT 1) token_expires_at,
              (SELECT c.data_center FROM connector_credentials c WHERE c.installation_id=i.id AND c.revoked_at IS NULL ORDER BY c.credential_version DESC LIMIT 1) data_center,
              COALESCE((SELECT to_jsonb(c.scopes) FROM connector_credentials c WHERE c.installation_id=i.id AND c.revoked_at IS NULL ORDER BY c.credential_version DESC LIMIT 1),'[]') scopes,
